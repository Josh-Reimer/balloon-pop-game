...
# no system JDK, and only a partial Android SDK. See CLAUDE.md for background
# on why each of these steps is needed on this specific kind of device.
#
...
compile_sdk="$(grep -oE 'compileSdk\s*=\s*[0-9]+' "$REPO_DIR/android/build.gradle" | grep -oE '[0-9]+')"
...
# --- 5. Build ---------------------------------------------------------------
cd "$REPO_DIR"
log "Running gradlew android:assembleDebug"
if ! sh ./gradlew android:assembleDebug --console=plain; then
  ...
  if [ "$found_new" = true ]; then
    sh ./gradlew android:assembleDebug --console=plain
  else
    exit 1
  fi
fi

apk="$(find "$REPO_DIR/android/build/outputs/apk/debug" -name '*.apk' | head -1)"
