...
All audio in the repo is synthesized — there are no recorded or licensed clips. The alien voice lines in `assets/voice/` are **placeholders**: procedurally generated mono 22 kHz WAVs (a buzzy glottal source shaped by gliding formant filters, with ring modulation for the alien timbre). They are meant to be replaced by hand-recorded, pitch/formant-shifted OGGs dropped in under the same basenames; `AlienVoiceManager` prefers `.ogg` so that swap needs no code change. Note that the ffmpeg on this Mac has no `libvorbis` and its native `vorbis` encoder is stereo-only, so mono OGG can't be produced here — hence WAV placeholders rather than OGG ones.
