...
**The overheat sequence** is a second, orthogonal state machine (`Overheat` enum: `NONE` → `SWARM` → `DOUSING`) running *inside* `State.PLAYING`, not a new `State` — the world keeps updating throughout, so it can't be modelled as a pause. Every `OVERHEAT_SHOT_LIMIT` (267) shots the barrel gives out: `beginOverheat()` locks out firing, clears the sky (balloons/asteroids already falling are `pop()`ed into clouds — worth no points, and popping suppresses their fall-past-the-bottom life penalty), and halts all normal spawning. In its place a swarm of parachuting aliens comes down, each one that reaches the ground unshot docking `OVERHEAT_ALIEN_PENALTY` points but never a life. The swarm keeps topping itself up until `OVERHEAT_ALIENS_REQUIRED` (3) aliens have *landed*, which is what guarantees the sequence terminates: firing is off, so only shots still in flight when the overheat began can thin the swarm. Landings are counted off `Alien.landedThisFrame` (a one-shot flag cleared at the top of `Alien.update`) rather than by scanning phases, and only for aliens with `holdOnLanding` set, so a plain bonus alien that happens to be mid-fall when the overheat starts is never charged for. Then `beginDousing()` sends the landed alien nearest the gun over with a bucket; the gun is frozen in place for that leg only (so it can't be dragged out from under the pour) while aiming stays free during `SWARM`. The pour is a real particle sim rather than a canned animation — see `WaterSplash` — and `GameScreen` steers its spout to `Alien.getBucketLipX()/Y()` every frame so the stream stays welded to the bucket as it rolls over. The bucket's tilt, its draining fill level and the lip position all derive from one `bucketLean()`, shared by the renderer and the emitter precisely so the water can't end up pouring from somewhere the bucket isn't; the lip is taken as whichever rim corner is currently *lower*, which stays correct at any tilt and in either direction. `endOverheat()` fires when the `WaterSplash` finishes, resetting `shotsSinceCooldown` and releasing the held aliens to waddle off — so the whole thing recurs every 267 shots. `bulletsFired` stays a lifetime counter; the separate `shotsSinceCooldown` drives the heat fill drawn behind the bullets chip, which is the only warning the player gets that an overheat is coming.

**`audio/`** — `AlienVoiceManager` plus its `AlienVoiceType` enum: the taunt an alien shouts when it's shot. Each type owns a pool of short clips in `assets/voice/`, loaded as `Sound` through the shared `AssetManager` created in `BalloonPopGame` (the only thing that uses it — the pop/fire sounds are still loaded directly, since they're swapped whenever the sfx style changes). Three separate mechanisms stop a kill streak turning into noise, and they are not redundant: at most one line *starts* per frame, queued lines are spaced by a stagger delay longer than the longest clip (the clips are spoken English, so two overlapping lines are unintelligible no matter how cleanly each was rendered — every spacing constant is derived from a single `LONGEST_CLIP`, since libGDX `Sound` can't report its own length), and each type has a cooldown during which new lines are **dropped rather than queued** — a backlog that played out over the following ten seconds would be worse than silence. Clip selection never repeats the clip it just played and jitters pitch per play, so a pool of eight doesn't go mechanical. Every line also surfaces its text via `getActiveInsults()` for `GameScreen` to draw at the death position: that is an accessibility feature, so it is driven by the same queue as the audio (keeping the two in sync through the stagger) and is deliberately **not** gated on the mute state. Clip paths resolve `.ogg` first and fall back to `.wav`, and a missing file is skipped rather than fatal — so the generated WAVs can be replaced by hand-recorded OGGs by filename alone, with no code change.
...
All audio in the repo is generated on this machine — there are no bought or licensed clips. The pop/fire sounds are pure synthesis. The alien voice lines in `assets/voice/` are mono 22 kHz WAVs built by `scripts/generate-alien-voices.sh` (macOS only — it needs `say`, plus ffmpeg): real text-to-speech for the words, then a *light* effect chain for the alien timbre — a pitch/formant shift via `asetrate`/`atempo`, a short comb-delay chorus, and a slow vibrato. Keep that chain mild when editing it. An earlier version of these clips was fully procedural (a buzzy glottal source through gliding formant filters, ring-modulated) and sounded genuinely alien but was unintelligible to an English speaker, which defeated the point of writing the lines; ring modulation in particular destroys the consonants. The script's line arrays must stay in the same order as the insult arrays in `AlienVoiceType`, since clip N is spoken while insult N is drawn.

Hand-recorded OGGs can still replace any clip by filename alone — `AlienVoiceManager` resolves `.ogg` before `.wav`. Note that the ffmpeg on this Mac has no `libvorbis` and its native `vorbis` encoder is stereo-only, so mono OGG can't be produced here — hence WAVs.
